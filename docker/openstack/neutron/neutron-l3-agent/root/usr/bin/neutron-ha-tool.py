...
def parse_args():
    ...
    for key in ['OS_USERNAME', 'OS_PASSWORD', 'OS_AUTH_URL',
        ...
        if key not in os.environ.keys():
            raise SystemExit("Your environment is missing '{}'".format(key))
...
def setup_logging(args):
    ...
    handler_file = logging.FileHandler(filename='/proc/1/fd/1')
    syslog_formatter = logging.Formatter('%(asctime)s %(name)s: %(pathname)s:%(lineno)d %(levelname)s %(message)s')
    handler_file.setFormatter(syslog_formatter)
